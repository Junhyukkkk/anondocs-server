 ...
 * - 일기 편집: /app/diaries/{diaryId}/edit
 *
 * 동작 방식:
 * 1. 클라이언트가 STOMP 메시지 전송
 * 2. 버전 기반 낙관적 락으로 동시성 제어
...
public class DiaryRealTimeController {
    ...
    public void createDiary(DiaryCreateMessageDto message, Principal principal) {
        try {
            ...
            log.error("일기 생성 실패 - 사용자: {}", principal.getName(), e);
    ...
    public void editDiary(@DestinationVariable Long diaryId,
        ...
        try {
            ...
            // 4. 버전 충돌 시: 해당 사용자에게만 에러 전송
            log.warn("버전 충돌 - 일기: {}, 요청 버전: {}, 현재 버전: {}",
                    diaryId, message.getVersion(), e.getCurrentVersion());
            ...
            // 5. 기타 예외: 해당 사용자에게만 에러 전송
            log.error("일기 편집 실패 - 일기: {}, 사용자: {}", diaryId, principal.getName(), e);
